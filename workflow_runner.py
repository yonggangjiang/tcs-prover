...
def chosen_model(value):
    """Require one supported model."""

    if value not in MODELS:
        raise Error("Choose Astra, Sol, Terra, or Luna.")
    return value
        ...
        if (
            ...
            if turn_id and compaction_key not in seen_compactions:
                ...
                emit(
                    ...
                    reasoningSummary="detailed", node=node_name,
...
def _check_actions(actions):
    ...
    for action in actions:
        if not isinstance(action, dict) or set(action) - {"when", "set", "merge", "emit", "memory"}:
...
def load_workflow(path):
    ...
    specific = {
        "structured": {"instructions", "inputs", "schema", "features", "require", "error"},
        "goal": {"task", "marker", "lifecycle", "resume", "stages"},
    }
...
def _model_call(node, prompts, state, options, visit):
    """Run a schema-constrained request using only the supplied node definition."""
    ...
    prompt = render_template(prompts[reference], values)
    return structured(
        prompt, node["schema"], node.get("stage", "model"),
        features=node.get("features", []), **_settings(node, options),
    )
...
def _execute(workflow, state, options, prompts):
    ...
    current, visits, revision = next(iter(nodes)), 0, 0
    sessions, revisions = {}, {}
            ...
            if node["run"] == "structured":
                ...
                if current not in sessions:
                    ...
                    session = author_session(
                        ...
                        author_limit_file=options.get("author_limit_file"), elapsed_seconds=options.get("elapsed_seconds", 0),
                        prompts={key: prompts[ref] for key, ref in lifecycle.items()},
...
def execute_workflows(paths, state, options=None):
    ...
    for path in paths:
        workflow = load_workflow(path)
        prepared.append((workflow, prepare(workflow, options)))
    for workflow, prompts in prepared:
        _execute(workflow, state, options, prompts)
        if state.get("failed"):
            break
    return state
...
def criticize(statement, solution, round_number, model=CRITIC_MODEL, effort=EFFORT, instructions=None, speed=DEFAULT_SPEED, prompts=None):
    options = {"model": model, "effort": effort, "speed": speed, "prompts": {"critic": instructions} if instructions is not None else {}}
    return _builtin_node("author_critic", "critic", {"statement": statement, "solution": solution}, options, round_number, prompts)


def finalize(statement, solution, model=WRITER_MODEL, effort=EFFORT, instructions=None, speed=DEFAULT_SPEED, prompts=None):
    state = {"statement": statement, "solution": solution}
    options = {"model": model, "effort": effort, "speed": speed, "prompts": {"final": instructions} if instructions is not None else {}}
    _builtin_node("clean_up", "latex_editor", state, options, prompts=prompts)
    return state["output"]


def polish(source, model=WRITER_MODEL, effort=EFFORT, instructions=None, speed=DEFAULT_SPEED, prompts=None):
    state = {"source": source}
    options = {"model": model, "effort": effort, "speed": speed, "prompts": {"final": instructions} if instructions is not None else {}}
    _builtin_node("clean_up", "latex_editor", state, options, prompts=prompts)
    return state["output"]
...
def run_goal(prompt, statement, critic_rounds=None, thinking_hours=DEFAULT_AUTHOR_HOURS,
             ...
             author_limit_file=None, elapsed_seconds=0):
    options = dict(
        ...
        speed=speed, author_limit_file=author_limit_file, elapsed_seconds=elapsed_seconds,
...
def main(argv=None):
    ...
    try:
        ...
        state = execute_workflows(args.workflows, {"input": source, "statement": source, "source": source}, options)
        if state.get("failed"):
            return 1
