...
DEFAULT_REVIEW_MODEL = runtime.MODEL
...
class App:
    ...
    def _workflow_options(
        ...
        """Normalize and validate settings shared by statement and LaTeX modes."""
        ...
        if include_review and review_model not in REVIEW_MODELS:
            raise ValueError("Choose Astra, Sol, Terra, or Luna for statement review.")
        if any(
            model not in MODELS
            for model in (author_model, critic_model, writer_model)
        ):
            raise ValueError("Choose Astra, Sol, Terra, or Luna for every proof stage.")
    ...
    def start_review(
        ...
        if "\0" in statement or "\0" in feedback:
            raise ValueError("The statement and revision feedback cannot contain NUL characters.")
    ...
    def _launch_workflow_locked(self, workflows, source, options, stage, node):
        """Launch YAML workflows; the caller holds ``self.lock``."""

        process = subprocess.Popen(
            [
                sys.executable, "-u", str(ROOT / "workflow_runner.py"),
                *(str(WORKFLOWS / name) for name in workflows),
                *options,
            ],
            stdin=subprocess.PIPE, stdout=subprocess.PIPE,
            stderr=subprocess.STDOUT, text=True, encoding="utf-8",
            errors="replace", bufsize=1,
            # Each workflow writes its durable proof files into this run only.
            cwd=self.run_dir,
            **isolated_process_options(),
        )
        token = self.active_token = object()
        ...
        self.state.update(
            ...
            activeNode=node, round=0, output="",
        )
        return process, token

    def _launch_solver_locked(self, statement):
        """Run author/critic and final cleanup; the caller holds ``self.lock``."""
        ...
        return self._launch_workflow_locked(
            ...
            [
                ...
                "--speed", self.state["speedMode"],
                "--author-prompt-file", str(self.run_dir / "prompts/author.txt"),
                "--critic-prompt-file", str(self.run_dir / "prompts/critic.txt"),
    ...
    def _launch_final_locked(self, source):
        """Run only the final LaTeX editor; the caller holds ``self.lock``."""

        return self._launch_workflow_locked(
            ("clean_up.yaml",), source,
            [
                "--writer-model", self.state["writerModel"],
                "--reasoning-effort", self.state["reasoningEffort"],
                "--writer-effort", self.state["writerEffort"],
        ...
        try:
            for line in process.stdout:
                ...
                if record.get("kind") == "failure_result":
                    problem = "Proof incomplete. See the saved failure summary for details."
                    with self.lock:
                        self.state["output"] = record.get("output", "")
                        final = True
...
class Server(ThreadingHTTPServer):
    ...
    def job_list(self):
        ...
        for app in apps:
            ...
            job["title"] = state["draft"].strip().split("\n", 1)[0]
            jobs.append(job)
